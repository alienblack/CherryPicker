CXX ?= g++
CXXFLAGS ?= -O3 -DNDEBUG -std=c++17 -Wall -Wextra -MMD -MP
BUILD := build
...
CERT_FLAGS := \
	...
	-DALLOW_STABLE_EXCHANGE_DEFAULT=0 \
	-DALLOW_EMPIRICAL_EXACT_DEFAULT=0 \
...
$(BUILD)/manytree_solver: src_manytree/manytree_pool_packer.cpp | $(BUILD)
	$(CXX) $(CXXFLAGS) $(CERT_FLAGS) $< -o $@
